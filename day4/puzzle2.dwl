...
fun playLosingBingo(remainingNumbers, boards) = do {
    var marked = boards map {
        board: $.board,
        marked: mark(remainingNumbers[0], $)
    }
    var losers = marked filter (b) -> ! boardWins(b)
    ---
    if (isEmpty(losers))
        { lastWinner: marked[0], lastNumber: remainingNumbers[0] }
    else
    (
        if (isEmpty(remainingNumbers)) 
            []
        else 
            playLosingBingo(remainingNumbers drop 1, losers)
    )
}
...
var lastWinnerData = playLosingBingo(drawnNumbers, gameBoards)
---

unmarkedSum: sum(unmarked(lastWinnerData.lastWinner) map (sum($))) * lastWinnerData.lastNumber
